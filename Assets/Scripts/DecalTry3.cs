...
public class DecalTry3 : MonoBehaviour {
	...
	void Update(){
		...
		if (continuous || lifeTime > 0) {
			if (lifeTimeCur > 0) {
				lifeTimeCur -= Time.deltaTime * GameManager.timeScale;
			} else {
				lifeTimeCur += lifeTime;
				if (!continuous){
			...
			if (texs.Count > 0) {
				if (deltaRot != 0){
					transform.Rotate(Vector3.forward, deltaRot * Time.deltaTime * GameManager.timeScale, Space.Self);
				}
				...
				//}
				if (curOffsetTime > 0){
					curOffsetTime -= Time.deltaTime * GameManager.timeScale;
				}else{
					if (randomOffset){
				...
				if (texs.Count > 1){
					if (texTimer > 0){
						texTimer -= Time.deltaTime * GameManager.timeScale;
					}else{
						if (randomTex){
